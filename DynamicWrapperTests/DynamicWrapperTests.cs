...
namespace HouseOfBilz
{
    ...
    public class DynamicWrapperTests
    {
        ...
        {
            bool IsTypeOf<T>();
            bool TypesAreSame<OneT, TwoT>();
            T GenericReturn<T>();
            T GenericParameter<T>(T value);
            bool GenericCompare<OneT, TwoT>(OneT one, TwoT two);
            IEnumerable<T> Return3<T>(Func<int, T> generate);
        }

        public class MyTester
        {
            public string Repsond(string message) { return message + " response"; }
            public int Return5() { return 5; }
            public void VoidMethod() { throw new Exception("VoidMethod"); }
            public void Add6ToParam(ref int param) { param = param + 6; }
            public void SetOutParamTo7(out int param) { param = 7; }
            public int Property { get; set; }

            public bool IsTypeOf<T>() { return typeof (T) == GetType(); }
            public bool TypesAreSame<OneT, TwoT>() { return typeof (OneT) == typeof (TwoT); }
            public T GenericReturn<T>() {return (T)Activator.CreateInstance(typeof (T)); }
            public T GenericParameter<T>(T value) {return value;}
            public bool GenericCompare<OneT, TwoT>(OneT one, TwoT two) {return one.Equals(two);}
        ...
        public interface IGenericInterface<T, F>
        {
            T GetT();
            F GetF();
        }
        ...
        {
            public T TVal = default(T);
            public F FVal = default(F);
        ...
        {
            var realObject = new GenericClass<string, int> {TVal = "FOO", FVal = 99};
        ...
        {
            var wrappedObject = _realObject.As<INotWrappable>();
        }
        ...
        public class MultiIF
        {
            ...
            public void Dispose() { _message = "Disposed"; }
            public string Foo() { return _message; }
        }
