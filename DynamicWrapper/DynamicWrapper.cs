...
namespace DynamicWrapper
{
  public class DynamicWrapper
  {
    ...
    private static void EmitGenericMethod(MethodInfo methodDefinition, Type objectType, TypeBuilder typeBuilder)
    {
      ...
      methodBuilder.DefineGenericParameters(methodDefinition.GetGenericArguments().Select(arg => arg.Name).ToArray());

      var ilGenerator = methodBuilder.GetILGenerator();

      ilGenerator.Emit(OpCodes.Ldarg_0);
      var sourceField = typeof(DynamicWrapperBase).GetField("RealObject", BindingFlags.Instance | BindingFlags.NonPublic);
